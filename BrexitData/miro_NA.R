...
# seed
# method=mean
miro_NA<-function(y,X,W,K,prior.var=5,maxT=5000,seed=42, nullClust=FALSE)
{
  set.seed(seed)
  d=ncol(y) ## number of events
  n=nrow(y) ## number of units
  NOT.na <- which(!is.na(y),arr.ind = T)
  X <- cbind(rep(1,n),X)
  ...
  ### filter NA
  rm.filt <- which(is.na(stack.y))
  stack.y <- stack.y[-rm.filt]
  stack.X <- stack.X[-rm.filt,]
  stack.W <- stack.W[-rm.filt]
  NN=length(stack.y)
  ...
  design=array(0,c(NN,(L+Q)*K,K_star))
  lin.pred=matrix(0,NN,K_star)
