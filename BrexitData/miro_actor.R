...
# prior.var, variance for the prior on the regression coefficients \beta
# maxT, number of MCMC iterations (default is 5000)
# seed
# method=mean
miro_A<-function(y,X,K,prior.var=5,maxT=5000,seed=42, nullClust=FALSE)
{
  set.seed(seed)
  ...
  design=array(0,c(n*d,(L)*K,K_star))
  lin.pred=matrix(0,n*d,K_star)
  prior.m.coeff=rep(0,(L)*K) 
  prior.var.coeff=prior.var*diag((L)*K)
  beta.chain=matrix(0,L*K,maxT)
  temp.coeff=c(rmvnorm(1,mean=prior.m.coeff,sigma=prior.var.coeff))
  for(h in 1:K_star){
    design[,,h]=kronecker(t(kron.u[h,]),stack.X)
