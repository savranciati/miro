...
# W, design matrix "n x Q" where Q number of event covariates (+interc)
# K, number of overlapping clusters
# prior.var, variance for the prior on the regression coefficients  \gamma (default is 5)
# maxT, number of MCMC iterations (default is 5000)
# seed
# method=mean
miro_AE<-function(y,W,K,prior.var=5,maxT=5000,seed=42, nullClust=FALSE)
{
  set.seed(seed)
  d=ncol(y) ## number of events
  n=nrow(y) ## number of units
  W <- cbind(rep(1,d),W)
  Q=ncol(W) ## number of event covariates (+int)
